...
// ================= GIAO DIỆN HTML =================
// Cập nhật form đăng nhập dùng student_id
...
// 2. Xử lý Đăng nhập - Đã sửa dùng student_id
app.post('/login', async (req, res) => {
    const { student_id, password } = req.body;

    try {
        // Truy vấn CSDL bằng trường student_id
        const user = await db.collection('users').findOne({ student_id: student_id, hash_password: password });

        if (!user) {
            return res.send(renderLogin("Sai Mã sinh viên hoặc mật khẩu!"));
        }

        // Lưu lịch sử đăng nhập[cite: 2]
...
// Giữ nguyên phần xử lý truy vấn[cite: 2]
app.post('/query', async (req, res) => {
    ...
    try {
        ...
        let result = Array.isArray(queryObj) 
            ? await db.collection(collection).aggregate(queryObj).toArray() 
            : await db.collection(collection).find(queryObj).toArray();
