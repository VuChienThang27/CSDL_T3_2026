...
function renderDashboard(user, queryStr = '', resultHTML = '') {
    ...
    <head>
        ...
        <style>
            ...
            .result-box { flex: 2; background: #2d2d2d; padding: 20px; border-radius: 8px; overflow-x: auto; }
            textarea { width: 95%; padding: 10px; margin-top: 5px; background: #3d3d3d; color: #a6e22e; border: 1px solid #555; font-family: 'Courier New', monospace; }
            button { padding: 10px 20px; background: #10b981; color: white; border: none; cursor: pointer; margin-top: 10px; border-radius: 4px; }
    ...
    <body>
        <div class="header">
            ...
            <h2>Xin chào, ${user.full_name} (${user.student_id})</h2>
        ...
        <div class="container">
            <div class="form-box">
                ...
                <form action="/query" method="POST">
                    <label>Nhập lệnh truy vấn (db.collection.find...):</label>
                    <textarea name="query" rows="15" placeholder='db.users.find({ status: "Active" })'>${queryStr || 'db.users.find({\n  status: "Active"\n})'}</textarea>
        ...
        <div class="theme-toggle">
            <button class="theme-btn sun-btn" onclick="setTheme('light')" title="Chế độ sáng">☀️</button>
            <button class="theme-btn moon-btn" onclick="setTheme('dark')" title="Chế độ tối">🌙</button>
        </div>
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.send(renderLogin("Sai Mã sinh viên hoặc mật khẩu!"));
        
        await db.collection('users').updateOne({ _id: user._id }, { 
            $push: { login_history: { login_at: new Date(), ip_address: req.ip || "127.0.0.1", status: "Success" } } 
        });
...
app.post('/query', async (req, res) => {
    ...
    try {
        const regex = /db\.([\w-]+)\.(find|aggregate)\(([\s\S]*?)\)(?:\.projection\(([\s\S]*?)\))?/;
        const match = rawQuery.match(regex);

        if (!match) throw new Error("Cú pháp không hợp lệ. Hãy dùng định dạng db.collection.find(...)");

        const collectionName = match[1];
        const method = match[2];
        const queryStr = match[3] || "{}";
        const projectionStr = match[4];

        const queryObj = new Function(`return (${queryStr})`)();
        const projectionObj = projectionStr ? new Function(`return (${projectionStr})`)() : null;

        let result;
        if (method === 'find') {
            let cursor = db.collection(collectionName).find(queryObj);
            if (projectionObj) cursor = cursor.project(projectionObj);
            result = await cursor.toArray();
        } else {
            result = await db.collection(collectionName).aggregate(queryObj).toArray();
        }
        ...
        } else { resultHTML = "<p>Không tìm thấy dữ liệu.</p>"; }

    } catch (err) { resultHTML = `<p style="color:#ff6b6b;"><b>Lỗi:</b> ${err.message}</p>`; }

    res.send(renderDashboard(req.session.user, rawQuery, resultHTML));
});
