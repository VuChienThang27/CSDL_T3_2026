        ...
        <div class="container">
            <div class="form-box">
                ...
                <form action="/query" method="POST">
                    <label>Nhập lệnh truy vấn (Ví dụ: db.users.find...):</label>
...
// 2. Xử lý Đăng nhập bằng student_id
...
// Xử lý Truy vấn Shell (Bỏ ô Collection)[cite: 2]
app.post('/query', async (req, res) => {
    ...
    try {
        // Regex tách thông tin từ chuỗi db.collection.method(...).projection(...)
        const regex = /db\.([\w-]+)\.(find|aggregate)\(([\s\S]*?)\)(?:\.projection\(([\s\S]*?)\))?/;
        const match = rawQuery.match(regex);

        if (!match) throw new Error("Cú pháp không hợp lệ. Ví dụ đúng: db.users.find({ status: 'Active' })");
        ...
        // Dùng Function() để parse thay vì JSON.parse để tránh lỗi dấu ngoặc/nháy
